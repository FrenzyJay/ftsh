...
/*   Updated: 2014/03/25 20:38:28 by llapillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_viewlist_job(t_job *lstjob)
{
	...
	while (42)
	{
		...
		ft_viewlist_process(curs_j->first_job);
		if (curs_j->next == NULL)
			break ;
		else
			curs_j = curs_j->next;
...
t_job		*ft_create_job(t_job *lst_j, int *process, char *cmd)
{
	t_job	*new;
...
void		ft_add_job(t_job **lst_j, int *process, char *cmd)
{
	...
	new = ft_create_job(lst_j, process, cmd);
	if (new)
	{
		if (!lst_j)
			lst_j = new;
